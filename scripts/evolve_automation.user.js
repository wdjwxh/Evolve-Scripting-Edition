...
(function ($) {
  ...
  var loggingTypes = {
    special: {
      id: "special",
      name: "Specials",
      settingKey: "log_special"
    },
    construction: {
      id: "construction",
      name: "Construction",
      settingKey: "log_construction"
    },
    multi_construction: {
      id: "multi_construction",
      name: "Multi-part Construction",
      settingKey: "log_multi_construction"
    },
    research: {
      id: "research",
      name: "Research",
      settingKey: "log_research"
    },
    spying: {
      id: "spying",
      name: "Spying",
      settingKey: "log_spying"
    },
    attack: {
      id: "attack",
      name: "Attack",
      settingKey: "log_attack"
    },
    mercenary: {
      id: "mercenary",
      name: "Mercenaries",
      settingKey: "log_mercenary"
    }
  };
  ...
  class GameLog {
    ...
    logSuccess(loggingType, text) {
      if (!settings[this._logEnabledSettingKey]) {
        return;
      }

      if (!settings[loggingType.settingKey]) {
        return;
      }
  ...
  class Job {
    ...
    constructor(id, name) {
      ...
      this._definition = null; // Settings
    ...
    breakpointEmployees(breakpoint, ignoreOverride) {
      ...
      let breakpointActual = this.breakpointMaxs[breakpoint]; // -1 equals unlimited up to the maximum available jobs for this job
    ...
    addWorkers(count) {
      ...
      if (getVueById(this._vueBinding) === undefined) {
        return false;
      }
      ...
      while (state.multiplier.remainder > 0) {
        ...
        (_getVueById = getVueById(this._vueBinding)) === null || _getVueById === void 0 ? void 0 : _getVueById.add();
      }
    ...
    removeWorkers(count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById2 = getVueById(this._vueBinding)) === null || _getVueById2 === void 0 ? void 0 : _getVueById2.sub();
      }
  ...
  class CraftingJob extends Job {
    ...
    addWorkers(count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById3 = getVueById(this._vueBinding)) === null || _getVueById3 === void 0 ? void 0 : _getVueById3.add(this._originalId);
      }
    ...
    removeWorkers(count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById4 = getVueById(this._vueBinding)) === null || _getVueById4 === void 0 ? void 0 : _getVueById4.sub(this._originalId);
      }
  ...
  class Action {
    ...
    constructor(name, tab, id, location) {
      ...
      if (this._elementId === "starDock-probes") {
        // Can't use buildings in the constructor as we are still creating them!
    ...
    click(count) {
      ...
      let retVal = true;
      let tempRetVal = true;
      let previousCount = this.count; // Not using state.multiplier here as there are affordability checks that are required before actioning
      ...
      for (let i = 0; i < count; i++) {
        if (retVal) {
          ...
          if (this === state.evolutions.Rna) {
            retVal = retVal && resources.RNA.currentQuantity < resources.RNA.maxQuantity;
          } else if (this === state.evolutions.Dna) {
            retVal = retVal && resources.DNA.currentQuantity < resources.DNA.maxQuantity;
          } else if (this === state.cityBuildings.Food) {
            retVal = retVal && resources.Food.currentQuantity < resources.Food.maxQuantity;
          } else if (this === state.cityBuildings.Lumber) {
            retVal = retVal && resources.Lumber.currentQuantity < resources.Lumber.maxQuantity;
          } else if (this === state.cityBuildings.Stone) {
            retVal = retVal && resources.Stone.currentQuantity < resources.Stone.maxQuantity;
          } else if (this === state.cityBuildings.Chrysotile) {
            retVal = retVal && resources.Chrysotile.currentQuantity < resources.Chrysotile.maxQuantity;
      ...
      || this === state.cityBuildings.Lumber || this === state.cityBuildings.Stone || this === state.cityBuildings.Slaughter || this === state.cityBuildings.Chrysotile || this === state.cityBuildings.SlaveMarket) {
        // Don't log buying slaves
        return retVal;
      }

      if (this.gameMax > 1 && this.gameMax < Number.MAX_SAFE_INTEGER) {
        // This build has segments that will be built
        state.log.logSuccess(loggingTypes.multi_construction, game.loc("build_success", [`${this.title} (${this.count}) `]));
      } else {
        state.log.logSuccess(loggingTypes.construction, game.loc("build_success", [`${this.title}`]));
      }
    ...
    addResourceConsumption(resource, rate) {
      this.consumption.resourceTypes.push({
        resource: resource,
        initialRate: rate,
        rate: rate
      });
  ...
  const Fibonacci = [5, 8, 13, 21, 34, 55, 89, 144, 233, 377, 610, 987, 1597, 2584, 4181, 6765, 10946, 17711, 28657, 46368, 75025, 121393, 196418, 317811, 514229, 832040, 1346269, 2178309, 3524578, 5702887, 9227465, 14930352, 24157817, 39088169, 63245986, 102334155, 165580141, 267914296, 433494437, 701408733];
  ...
  class MinorTraitManager {
    ...
    sortByPriority() {
      ...
      this._managedPriorityList.sort(function (a, b) {
        return a.priority - b.priority;
      });
    }
    ...
    tryBuyWithGenes(traitName, count) {
      ...
      if (getVueById(this._traitVueBinding) === undefined) {
        return false;
      }
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById5 = getVueById(this._traitVueBinding)) === null || _getVueById5 === void 0 ? void 0 : _getVueById5.gene(traitName);
      }
  ...
  class Resource {
    ...
    increaseEjection(count) {
      if (getVueById(this._ejectorVueBinding) === undefined) {
        return false;
      }
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById6 = getVueById(this._ejectorVueBinding)) === null || _getVueById6 === void 0 ? void 0 : _getVueById6.ejectMore(this.id);
      }
    }
    /**
     * @param {number} count
    ...
    decreaseEjection(count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById7 = getVueById(this._ejectorVueBinding)) === null || _getVueById7 === void 0 ? void 0 : _getVueById7.ejectLess(this.id);
      }
    }
    /**
     * @param {boolean} buy
    ...
    get maxQuantity() {
      ...
      let storageNode = document.getElementById(this._storageCountId); // 2 possibilities:
    ...
    tryAssignCrate(count) {
      if (getVueById(this._stackVueBinding) === undefined) {
        return false;
      }
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById8 = getVueById(this._stackVueBinding)) === null || _getVueById8 === void 0 ? void 0 : _getVueById8.addCrate(this.id);
      }
    ...
    tryUnassignCrate(count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById9 = getVueById(this._stackVueBinding)) === null || _getVueById9 === void 0 ? void 0 : _getVueById9.subCrate(this.id);
      }
    ...
    tryAssignContainer(count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById10 = getVueById(this._stackVueBinding)) === null || _getVueById10 === void 0 ? void 0 : _getVueById10.addCon(this.id);
      }
    ...
    tryUnassignContainer(count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById11 = getVueById(this._stackVueBinding)) === null || _getVueById11 === void 0 ? void 0 : _getVueById11.subCon(this.id);
      }
    ...
    tryCraftX(count) {
      ...
      if (game.global.race[challengeNoCraft]) {
        return false;
      }
    ...
    isClickable() {
      ...
      if (this.count === 0) {
        ...
        if (resources.Population.currentQuantity < 20 || resources.Population.currentQuantity !== resources.Population.maxQuantity) {
          ...
          let clickable = game.global.city.s_alter.rage < 3600 || game.global.city.s_alter.regen < 3600 || game.global.city.s_alter.mind < 3600 || game.global.city.s_alter.mine < 3600;
  ...
  class Smelter extends Action {
    ...
    sortByPriority() {
      this._fuelPriorityList.sort(function (a, b) {
        return a.priority - b.priority;
      });
    }
    ...
    increaseFuel(fuelType, count) {
      if (getVueById(this._industryVueBinding) === undefined) {
        return false;
      }
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById12 = getVueById(this._industryVueBinding)) === null || _getVueById12 === void 0 ? void 0 : _getVueById12.addFuel(type);
      }
    ...
    decreaseFuel(fuelType, count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById13 = getVueById(this._industryVueBinding)) === null || _getVueById13 === void 0 ? void 0 : _getVueById13.subFuel(type);
      }
    ...
    increaseSmelting(smeltingType, count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        if (smeltingType === SmelterSmeltingTypes.Iron) {
          ...
          (_getVueById14 = getVueById(this._industryVueBinding)) === null || _getVueById14 === void 0 ? void 0 : _getVueById14.ironSmelting();
        }

        if (smeltingType === SmelterSmeltingTypes.Steel) {
          var _getVueById15;

          (_getVueById15 = getVueById(this._industryVueBinding)) === null || _getVueById15 === void 0 ? void 0 : _getVueById15.steelSmelting();
        }
      }
  ...
  class Factory extends Action {
    ...
    get productionOptions() {
      if (this._productionOptions === null) {
        ...
        this._productionOptions.push({
          seq: 1,
          goods: FactoryGoods.LuxuryGoods,
      ...
      this._productionOptions.sort(function (a, b) {
    ...
    increaseProduction(production, count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById16 = getVueById(this._industryVueBinding)) === null || _getVueById16 === void 0 ? void 0 : _getVueById16.addItem(production);
      }
    ...
    decreaseProduction(production, count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById17 = getVueById(this._industryVueBinding)) === null || _getVueById17 === void 0 ? void 0 : _getVueById17.subItem(production);
      }
  ...
  class MiningDroid extends Action {
    ...
    increaseProduction(production, count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById18 = getVueById(this._industryVueBinding)) === null || _getVueById18 === void 0 ? void 0 : _getVueById18.addItem(production);
      }
    ...
    decreaseProduction(production, count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById19 = getVueById(this._industryVueBinding)) === null || _getVueById19 === void 0 ? void 0 : _getVueById19.subItem(production);
      }
    ...
    increaseFuel(fuelType, count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        if (fuelType === GrapheneFuelTypes.Lumber) {
          ...
          (_getVueById20 = getVueById(this._industryVueBinding)) === null || _getVueById20 === void 0 ? void 0 : _getVueById20.addWood();
        }

        if (fuelType === GrapheneFuelTypes.Coal) {
          var _getVueById21;

          (_getVueById21 = getVueById(this._industryVueBinding)) === null || _getVueById21 === void 0 ? void 0 : _getVueById21.addCoal();
        }

        if (fuelType === GrapheneFuelTypes.Oil) {
          var _getVueById22;

          (_getVueById22 = getVueById(this._industryVueBinding)) === null || _getVueById22 === void 0 ? void 0 : _getVueById22.addOil();
        }
      }
    ...
    decreaseFuel(fuelType, count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        if (fuelType === GrapheneFuelTypes.Wood) {
          ...
          (_getVueById23 = getVueById(this._industryVueBinding)) === null || _getVueById23 === void 0 ? void 0 : _getVueById23.subWood();
        }

        if (fuelType === GrapheneFuelTypes.Coal) {
          var _getVueById24;

          (_getVueById24 = getVueById(this._industryVueBinding)) === null || _getVueById24 === void 0 ? void 0 : _getVueById24.subCoal();
        }

        if (fuelType === GrapheneFuelTypes.Oil) {
          var _getVueById25;

          (_getVueById25 = getVueById(this._industryVueBinding)) === null || _getVueById25 === void 0 ? void 0 : _getVueById25.subOil();
        }
      }
  ...
  class SpaceDock extends Action {
    ...
    isOptionsCached() {
      ...
      if (!state.spaceBuildings.GasSpaceDockProbe.isOptionsCached() || game.global.tech['genesis'] >= 5 && !state.spaceBuildings.GasSpaceDockShipSegment.isOptionsCached() || game.global.tech['genesis'] === 6 && !state.spaceBuildings.GasSpaceDockPrepForLaunch.isOptionsCached() || game.global.tech['genesis'] >= 7 && !state.spaceBuildings.GasSpaceDockLaunch.isOptionsCached()) {
        return false;
      }
  ...
  var governmentTypes = {
    anarchy: {
      id: "anarchy",
      name: function () {
        return game.loc("govern_anarchy");
      }
    },
    ...
    autocracy: {
      id: "autocracy",
      name: function () {
        return game.loc("govern_autocracy");
      }
    },
    democracy: {
      id: "democracy",
      name: function () {
        return game.loc("govern_democracy");
      }
    },
    oligarchy: {
      id: "oligarchy",
      name: function () {
        return game.loc("govern_oligarchy");
      }
    },
    theocracy: {
      id: "theocracy",
      name: function () {
        return game.loc("govern_theocracy");
      }
    },
    republic: {
      id: "republic",
      name: function () {
        return game.loc("govern_republic");
      }
    },
    socialist: {
      id: "socialist",
      name: function () {
        return game.loc("govern_socialist");
      }
    },
    corpocracy: {
      id: "corpocracy",
      name: function () {
        return game.loc("govern_corpocracy");
      }
    },
    technocracy: {
      id: "technocracy",
      name: function () {
        return game.loc("govern_technocracy");
      }
    },
    federation: {
      id: "federation",
      name: function () {
        return game.loc("govern_federation");
      }
    }
  };
  ...
  class GovernmentManager {
    ...
    setGovernment(government) {
      ...
      if (!this.isGovernmentUnlocked(government)) {
        return;
      }

      if (government === governmentTypes.anarchy.id) {
        return;
      }
  ...
  var espionageTypes = {
    ...
    influence: {
      id: "influence",
      name: function () {
        return game.loc("civics_spy_influence");
      }
    },
    sabotage: {
      id: "sabotage",
      name: function () {
        return game.loc("civics_spy_sabotage");
      }
    },
    incite: {
      id: "incite",
      name: function () {
        return game.loc("civics_spy_incite");
      }
    },
    round_robin: {
      id: "rrobin",
  ...
  class SpyManager {
    ...
    isUnlocked() {
      ...
      if (!game.global.tech['spy'] || node === null || node.style.display === "none") {
        return false;
      }

      let foreignVue = getVueById("foreign");

      if (foreignVue === undefined || !foreignVue.vis()) {
        return false;
      }
    ...
    performEspionage(govIndex, espionageId) {
      ...
      if (espionageId === espionageTypes.none.id) {
        return;
      }
      ...
      let optionsSpan = document.querySelector(`#gov${govIndex} div span:nth-child(3)`); // @ts-ignore
      ...
      if (espionageId === espionageTypes.round_robin.id) {
        ...
        let missionIndex = this._roundRobinIndex[govIndex]; // We're NOT looping througn the missions here. We are just looping through the number of missions that there are.
        // Round Robin is keeping track of the current mission itself in this._govMissionIndex[]

        for (let i = 0; i < this._missions.length; i++) {
          missionIndex++;

          if (missionIndex > this._missions.length - 1) {
            missionIndex = 0;
  ...
  class ModalWindowManager {
    ...
    checkCallbacks() {
      ...
      let windowName = this.currentModalWindowTitle; //console.log("windowname " + windowName);
  ...
  class WarManager {
    ...
    get hellSoulForgeSoldiers() {
      if (!game.global.portal.soul_forge || !game.global.portal.soul_forge.on) return 0; // Taken from the game code, so should give the same result
    ...
    getSoldiersForAttackRating(targetRating) {
      ...
      let soldiers = 10;
      singleSoldierAttackRating = game.armyRating(soldiers, this._textArmy, 0) / soldiers;
      let maxSoldiers = Math.ceil(targetRating / singleSoldierAttackRating); // At 10 soldiers there's no hivemind bonus or malus, and the malus gets up to 50%, so start with up to 2x soldiers below 10
    ...
    switchToBestAttackType(govOccupyIndex, govAttackIndex, govUnoccupyIndex) {
      ...
      if (this.selectedGovAttackIndex === -1) {
        ...
        if (govAttackIndex >= 0) {
          ...
          this.selectedGovAttackIndex = govAttackIndex; //console.log("setting gov index to govAttackIndex")
        } else if (govUnoccupyIndex >= 0) {
          this.selectedGovAttackIndex = govUnoccupyIndex; //console.log("setting gov index to govUnoccupyIndex")
        }
      ...
      for (let i = maxCampaignIndex; i >= 0; i--) {
        ...
        let campaignMaxAttackRating = campaign.getMaxRatingForGov(this.selectedGovAttackIndex); // We are within our ranges so this is the required tactic
    ...
    updateHell() {
      if (!this.isHellUnlocked()) return; // Determine the number of powered attractors
      ...
      if (settings.hellHandleAttractors && game.global.portal.attractor && game.global.portal.fortress.threat < settings.hellAttractorTopThreat && game.global.portal.fortress.assigned > 0) {
        ...
        if (game.global.portal.fortress.threat > settings.hellAttractorBottomThreat && settings.hellAttractorTopThreat > settings.hellAttractorBottomThreat) {
          this.hellAttractorMax = Math.floor(this.hellAttractorMax * (settings.hellAttractorTopThreat - game.global.portal.fortress.threat) / (settings.hellAttractorTopThreat - settings.hellAttractorBottomThreat));
        }
      ...
      let targetHellPatrolSize = 0; // First handle not having enough soldiers, then handle patrols
      // Only go into hell at all if walls are maxed and soldiers are close to full, or we are already there

      if (settings.hellHandlePatrolCount && this.maxSoldiers > settings.hellHomeGarrison + settings.hellMinSoldiers && (this.hellSoldiers > settings.hellMinSoldiers || this.availableSoldiers >= this.maxSoldiers * settings.hellMinSoldiersPercent / 100 && game.global.portal.fortress.walls === 100)) {
        ...
        let availableHellSoldiers = targetHellSoldiers - this.hellSoulForgeSoldiers; // Determine target hell garrison size
        ...
        if (settings.hellHandlePatrolSize) {
          let patrolRating = game.global.portal.fortress.threat * settings.hellPatrolThreatPercent / 100; //tempRating1 = patrolRating;
          ...
          patrolRating = Math.max(patrolRating, settings.hellPatrolMinRating); // Increase patrol attack rating if alive soldier count is low to reduce patrol losses
          ...
          targetHellPatrolSize = this.getSoldiersForAttackRating(patrolRating); // If patrol size is larger than available soldiers, send everyone available instead of 0
        ...
        targetHellPatrols = Math.floor((availableHellSoldiers - hellGarrison) / targetHellPatrolSize); // Special logic for small number of patrols
  ...
  class JobManager {
    ...
    sortByPriority() {
      ...
      this.craftingJobs.sort(function (a, b) {
        return a.priority - b.priority;
      });
    }
  ...
  class BuildingManager {
    ...
    sortByPriority() {
      ...
      this._statePriorityList.sort(function (a, b) {
        return a.priority - b.priority;
      });

      this._managedStatePriorityList.sort(function (a, b) {
        return a.priority - b.priority;
      });
    }
    ...
    managedStatePriorityList() {
      ...
      if (this._managedStatePriorityList.length === 0) {
        ...
        for (let i = 0; i < this._statePriorityList.length; i++) {
          const building = this._statePriorityList[i]; // If the building doesn't yet have state then it doesn't need to be managed (either not unlocked or tech for state not unlocked)
  ...
  class ProjectManager {
    ...
    managedPriorityList() {
      ...
      if (this._managedPriorityList.length === 0) {
        ...
        for (let i = 0; i < this.priorityList.length; i++) {
          const project = this.priorityList[i]; //console.log(project.id + " unlocked= " + project.isUnlocked() + " autoBuildEnabled= " + project.autoBuildEnabled + " autoSpace= " + settings.autoSpace)
  ...
  class MarketManager {
    ...
    sortByPriority() {
      this.priorityList.sort(function (a, b) {
        return a.marketPriority - b.marketPriority;
      });

      this._sortedTradeRouteSellList.sort(function (a, b) {
        return a.marketPriority - b.marketPriority;
      });
    }
    /** @param {Resource} resource */
    ...
    getSortedTradeRouteSellList() {
      ...
      if (this._sortedTradeRouteSellList.length === 0) {
        ...
        this._sortedTradeRouteSellList.sort(function (a, b) {
          return b.currentTradeRouteSellPrice - a.currentTradeRouteSellPrice;
        });
      }
    ...
    addTradeRoutes(resource, count) {
      if (getVueById(resource.marketVueBinding) === undefined) {
        return false;
      }
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById26 = getVueById(resource.marketVueBinding)) === null || _getVueById26 === void 0 ? void 0 : _getVueById26.autoBuy(resource.id);
      }
    ...
    removeTradeRoutes(resource, count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById27 = getVueById(resource.marketVueBinding)) === null || _getVueById27 === void 0 ? void 0 : _getVueById27.autoSell(resource.id);
      }
  ...
  class StorageManager {
    ...
    tryConstructCrate(count) {
      ...
      if (getVueById(this._storageVueBinding) === undefined) {
        return false;
      }
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById28 = getVueById(this._storageVueBinding)) === null || _getVueById28 === void 0 ? void 0 : _getVueById28.crate();
      }
    ...
    tryConstructContainer(count) {
      ...
      while (state.multiplier.remainder > 0) {
        ...
        state.multiplier.setMultiplier();
        (_getVueById29 = getVueById(this._storageVueBinding)) === null || _getVueById29 === void 0 ? void 0 : _getVueById29.container();
      }
  ...
  class Technology {
    ...
    resourceCost(resourceId) {
      if (!this.definition.cost[resourceId]) {
        return 0;
      }
  ...
  class Trigger {
    ...
    updateRequirementId(requirementId) {
      ...
      this.requirementId = requirementId;
      this.complete = false; // changing id doesn't change other requirements
    }
    /** @param {number} requirementCount */
    ...
    updateRequirementCount(requirementCount) {
      ...
      this.requirementCount = requirementCount;
      this.complete = false; // changing count doesn't change other requirements
    }
    /** @param {string} actionType */
  ...
  class TriggerManager {
    ...
    get targetTriggers() {
      if (this._targetTriggers === null) {
        this._targetTriggers = []; //console.log(this.priorityList.length)
    ...
    */
    ...
    costsConflict(origTriggerCosts, origActionCosts, multiplier = 1) {
      ...
      const actionCosts = game.adjustCosts(origActionCosts); // console.log("triggerCosts");
      ...
      let triggerBlocksKnowledge = false; // @ts-ignore
    ...
    actionConflicts(trigger) {
      ...
      for (let i = 0; i < this._targetTriggers.length; i++) {
        const targetTrigger = this._targetTriggers[i]; //@ts-ignore
    ...
    projectConflicts(project) {
      for (let i = 0; i < this.targetTriggers.length; i++) {
        const targetTrigger = this.targetTriggers[i]; //@ts-ignore
  ...
  var raceAchievementList = [races.antid, races.mantis, races.scorpid, races.human, races.orc, races.elven, races.troll, races.ogre, races.cyclops, races.kobold, races.goblin, races.gnome, races.cath, races.wolven, races.centaur, races.balorg, races.imp, races.seraph, races.unicorn, races.arraak, races.pterodacti, races.dracnid, races.tortoisan, races.gecko, races.slitheryn, races.sharkin, races.octigoran, races.entish, races.cacti, races.pinguicula, races.sporgar, races.shroomi, races.moldling, races.junker, races.dryad, races.satyr, races.phoenix, races.salamander, races.yeti, races.wendigo, races.tuskin, races.kamel, races.custom];
  ...
  var resources = {
    ...
    Population: new Resource("Population", "res", "Population", false, false, -1, false, -1, false),
  ...
  var state = {
    ...
    evolutions: {
      Rna: new EvolutionAction("RNA", "evolution", "rna", ""),
      Dna: new EvolutionAction("DNA", "evolution", "dna", ""),
      Membrane: new EvolutionAction("Membrane", "evolution", "membrane", ""),
      Organelles: new EvolutionAction("Organelles", "evolution", "organelles", ""),
      Nucleus: new EvolutionAction("Nucleus", "evolution", "nucleus", ""),
      EukaryoticCell: new EvolutionAction("Eukaryotic Cell", "evolution", "eukaryotic_cell", ""),
      Mitochondria: new EvolutionAction("Mitochondria", "evolution", "mitochondria", ""),
      SexualReproduction: new EvolutionAction("", "evolution", "sexual_reproduction", ""),
      Phagocytosis: new EvolutionAction("", "evolution", "phagocytosis", ""),
      Multicellular: new EvolutionAction("", "evolution", "multicellular", ""),
      BilateralSymmetry: new EvolutionAction("", "evolution", "bilateral_symmetry", ""),
      Arthropods: new EvolutionAction("", "evolution", "athropods", ""),
      Sentience: new EvolutionAction("", "evolution", "sentience", ""),
      Mantis: new EvolutionAction("", "evolution", "mantis", ""),
      Scorpid: new EvolutionAction("", "evolution", "scorpid", ""),
      Antid: new EvolutionAction("Antid", "evolution", "antid", ""),
      Mammals: new EvolutionAction("", "evolution", "mammals", ""),
      Humanoid: new EvolutionAction("", "evolution", "humanoid", ""),
      Human: new EvolutionAction("", "evolution", "human", ""),
      Orc: new EvolutionAction("", "evolution", "orc", ""),
      Elven: new EvolutionAction("", "evolution", "elven", ""),
      Valdi: new EvolutionAction("", "evolution", "junker", ""),
      // junker challenge
      Gigantism: new EvolutionAction("", "evolution", "gigantism", ""),
      Troll: new EvolutionAction("", "evolution", "troll", ""),
      Ogre: new EvolutionAction("", "evolution", "ogre", ""),
      Cyclops: new EvolutionAction("", "evolution", "cyclops", ""),
      Dwarfism: new EvolutionAction("", "evolution", "dwarfism", ""),
      Kobold: new EvolutionAction("", "evolution", "kobold", ""),
      Goblin: new EvolutionAction("", "evolution", "goblin", ""),
      Gnome: new EvolutionAction("", "evolution", "gnome", ""),
      Animalism: new EvolutionAction("", "evolution", "animalism", ""),
      Cath: new EvolutionAction("", "evolution", "cath", ""),
      Wolven: new EvolutionAction("", "evolution", "wolven", ""),
      Centaur: new EvolutionAction("", "evolution", "centaur", ""),
      Demonic: new EvolutionAction("", "evolution", "demonic", ""),
      // hellscape only
      Balorg: new EvolutionAction("", "evolution", "balorg", ""),
      Imp: new EvolutionAction("", "evolution", "imp", ""),
      Celestial: new EvolutionAction("", "evolution", "celestial", ""),
      // eden only
      Seraph: new EvolutionAction("", "evolution", "seraph", ""),
      Unicorn: new EvolutionAction("", "evolution", "unicorn", ""),
      Fey: new EvolutionAction("", "evolution", "fey", ""),
      // forest only
      Dryad: new EvolutionAction("", "evolution", "dryad", ""),
      Satyr: new EvolutionAction("", "evolution", "satyr", ""),
      Heat: new EvolutionAction("", "evolution", "heat", ""),
      // volcanic only
      Phoenix: new EvolutionAction("", "evolution", "phoenix", ""),
      Salamander: new EvolutionAction("", "evolution", "salamander", ""),
      Polar: new EvolutionAction("", "evolution", "polar", ""),
      // tundra only
      Yeti: new EvolutionAction("", "evolution", "yeti", ""),
      Wendigo: new EvolutionAction("", "evolution", "wendigo", ""),
      Sand: new EvolutionAction("", "evolution", "sand", ""),
      // desert only
      Tuskin: new EvolutionAction("", "evolution", "tuskin", ""),
      Kamel: new EvolutionAction("", "evolution", "kamel", ""),
      Eggshell: new EvolutionAction("", "evolution", "eggshell", ""),
      Endothermic: new EvolutionAction("", "evolution", "endothermic", ""),
      Arraak: new EvolutionAction("", "evolution", "arraak", ""),
      Pterodacti: new EvolutionAction("", "evolution", "pterodacti", ""),
      Dracnid: new EvolutionAction("", "evolution", "dracnid", ""),
      Ectothermic: new EvolutionAction("", "evolution", "ectothermic", ""),
      Tortoisan: new EvolutionAction("", "evolution", "tortoisan", ""),
      Gecko: new EvolutionAction("", "evolution", "gecko", ""),
      Slitheryn: new EvolutionAction("", "evolution", "slitheryn", ""),
      Aquatic: new EvolutionAction("", "evolution", "aquatic", ""),
      // ocean only
      Sharkin: new EvolutionAction("", "evolution", "sharkin", ""),
      Octigoran: new EvolutionAction("", "evolution", "octigoran", ""),
      Custom: new EvolutionAction("", "evolution", "custom", ""),
      Chloroplasts: new EvolutionAction("", "evolution", "chloroplasts", ""),
      //Multicellular: new EvolutionAction("", "evolution", "multicellular", ""),
      Poikilohydric: new EvolutionAction("", "evolution", "poikilohydric", ""),
      Bryophyte: new EvolutionAction("", "evolution", "bryophyte", ""),
      Entish: new EvolutionAction("", "evolution", "entish", ""),
      Cacti: new EvolutionAction("", "evolution", "cacti", ""),
      Pinguicula: new EvolutionAction("", "evolution", "pinguicula", ""),
      Chitin: new EvolutionAction("", "evolution", "chitin", ""),
      //Multicellular: new EvolutionAction("", "evolution", "multicellular", ""),
      Spores: new EvolutionAction("", "evolution", "spores", ""),
      //Bryophyte: new EvolutionAction("", "evolution", "bryophyte", ""),
      Sporgar: new EvolutionAction("", "evolution", "sporgar", ""),
      Shroomi: new EvolutionAction("", "evolution", "shroomi", ""),
      Moldling: new EvolutionAction("", "evolution", "moldling", ""),
      //Bunker: new EvolutionAction("", "evolution", "bunker", ""),
      Bunker: new ChallengeEvolutionAction("", "evolution", "bunker", "", ""),
      Plasmid: new ChallengeEvolutionAction("Plasmid", "evolution", "plasmid", "", "no_plasmid"),
      Trade: new ChallengeEvolutionAction("Trade", "evolution", "trade", "", "no_trade"),
      Craft: new ChallengeEvolutionAction("Craft", "evolution", "craft", "", "no_craft"),
      Crispr: new ChallengeEvolutionAction("Crispr", "evolution", "crispr", "", "no_crispr"),
      Mastery: new ChallengeEvolutionAction("Mastery", "evolution", "mastery", "", "weak_mastery"),
      Joyless: new ChallengeEvolutionAction("Joyless", "evolution", "joyless", "", "joyless"),
      Decay: new ChallengeEvolutionAction("Decay", "evolution", "decay", "", "decay"),
      Junker: new ChallengeEvolutionAction("Junker", "evolution", "junker", "", ""),
      Steelen: new ChallengeEvolutionAction("Steelen", "evolution", "steelen", "", "steelen"),
      EmField: new ChallengeEvolutionAction("EM Field", "evolution", "emfield", "", "emfield"),
      Cataclysm: new ChallengeEvolutionAction("Cataclysm", "evolution", "cataclysm", "", "cataclysm")
    },
    // weak_mastery
    ...
    cityBuildings: {
      ...
      SacrificialAltar: new SacrificialAlter(),
      // special click properties
  ...
  function initialiseState() {
    resetMarketState();
    resetStorageState(); // Construct craftable resource list
    ...
    resources.Scarletite.resourceRequirements.push(new ResourceRequirement(resources.Orichalcum, 500)); // Lets set our crate / container resource requirements

    resources.Crates.resourceRequirements.push(new ResourceRequirement(resources.Plywood, 10));
    resources.Containers.resourceRequirements.push(new ResourceRequirement(resources.Steel, 125)); // Construct all resource list
    ...
    resetJobState(); // Construct city builds list
    ...
    state.cityBuildings.TouristCenter.addResourceConsumption(resources.Food, 50); // Construct space buildings list
    ...
    state.spaceBuildings.AlphaMegaFactory.addResourceConsumption(resources.Deuterium, 5); // These are buildings which are specified as powered in the actions definition game code but aren't actually powered in the main.js powered calculations
    ////////////////////
  ...
  function resetGeneralSettings() {// None at the moment - moved to government settings
  }
  ...
  function resetMinorTraitSettings() {// None currently
  }
  ...
  function resetProductionState() {
    ...
    state.cityBuildings.Smelter.addFuelToPriorityList(new SmelterFuel(resources.Lumber)); // Factory settings
  ...
  initialiseState();
  var settingsSections = ["generalSettingsCollapsed", "prestigeSettingsCollapsed", "evolutionSettingsCollapsed", "researchSettingsCollapsed", "marketSettingsCollapsed", "storageSettingsCollapsed", "productionSettingsCollapsed", "warSettingsCollapsed", "hellSettingsCollapsed", "jobSettingsCollapsed", "buildingSettingsCollapsed", "projectSettingsCollapsed", "governmentSettingsCollapsed", "loggingSettingsCollapsed", "minorTraitSettingsCollapsed"];
  ...
  function updateStateFromSettings() {
    ...
    for (let i = 0; i < state.marketManager.priorityList.length; i++) {
      ...
      if (settings.hasOwnProperty(settingKey)) {
        resource.marketPriority = parseInt(settings[settingKey]);
      } else {
        settings[settingKey] = resource.marketPriority;
      }

      settingKey = 'buy' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.autoBuyEnabled = settings[settingKey];
      } else {
        settings[settingKey] = resource.autoBuyEnabled;
      }

      settingKey = 'res_buy_r_' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.autoBuyRatio = parseFloat(settings[settingKey]);
      } else {
        settings[settingKey] = resource.autoBuyRatio;
      }

      settingKey = 'sell' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.autoSellEnabled = settings[settingKey];
      } else {
        settings[settingKey] = resource.autoSellEnabled;
      }

      settingKey = 'res_sell_r_' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.autoSellRatio = parseFloat(settings[settingKey]);
      } else {
        settings[settingKey] = resource.autoSellRatio;
      }

      settingKey = 'res_trade_buy_' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.autoTradeBuyEnabled = settings[settingKey];
      } else {
        settings[settingKey] = resource.autoTradeBuyEnabled;
      }

      settingKey = 'res_trade_buy_mtr_' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.autoTradeBuyRoutes = parseInt(settings[settingKey]);
      } else {
        settings[settingKey] = resource.autoTradeBuyRoutes;
      }

      settingKey = 'res_trade_sell_' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.autoTradeSellEnabled = settings[settingKey];
      } else {
        settings[settingKey] = resource.autoTradeSellEnabled;
      }

      settingKey = 'res_trade_sell_mps_' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.autoTradeSellMinPerSecond = parseFloat(settings[settingKey]);
      } else {
        settings[settingKey] = resource.autoTradeSellMinPerSecond;
      }

      settingKey = 'res_storage' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.autoStorageEnabled = settings[settingKey];
      } else {
        settings[settingKey] = resource.autoStorageEnabled;
      }

      settingKey = 'res_storage_w_' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.autoStorageWeighting = parseFloat(settings[settingKey]);
      } else {
        settings[settingKey] = resource.autoStorageWeighting;
      }

      settingKey = 'res_storage_p_' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource.storagePriority = parseFloat(settings[settingKey]);
      } else {
        settings[settingKey] = resource.storagePriority;
      }

      settingKey = 'res_crates_m_' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource._autoCratesMax = parseInt(settings[settingKey]);
      } else {
        settings[settingKey] = resource._autoCratesMax;
      }

      settingKey = 'res_containers_m_' + resource.id;

      if (settings.hasOwnProperty(settingKey)) {
        resource._autoContainersMax = parseInt(settings[settingKey]);
      } else {
        settings[settingKey] = resource._autoContainersMax;
      }
    }
    ...
    for (let i = 0; i < state.minorTraitManager.priorityList.length; i++) {
      ...
      if (settings.hasOwnProperty(settingKey)) {
        trait.autoMinorTraitEnabled = settings[settingKey];
      } else {
        settings[settingKey] = trait.autoMinorTraitEnabled;
      }

      settingKey = 'mTrait_w_' + trait.traitName;

      if (settings.hasOwnProperty(settingKey)) {
        trait.autoMinorTraitWeighting = parseFloat(settings[settingKey]);
      } else {
        settings[settingKey] = trait.autoMinorTraitWeighting;
      }

      settingKey = 'mTrait_p_' + trait.traitName;

      if (settings.hasOwnProperty(settingKey)) {
        trait.priority = parseFloat(settings[settingKey]);
      } else {
        settings[settingKey] = trait.priority;
      }
    }

    state.minorTraitManager.sortByPriority(); // Retrieve settings for crafting resources
    ...
    state.buildingManager.sortByPriority(); // Retrieve settings for assigning jobs
  ...
  function updateSettingsFromState() {
    updateStandAloneSettings();
    settings.triggers = state.triggerManager.priorityList; // Remove old building settings... We had to update these with the prefix as well as building ids started to have duplicates

    for (let i = 0; i < state.buildingManager.priorityList.length; i++) {
      const building = state.buildingManager.priorityList[i];

      if (settings.hasOwnProperty('bat' + building.id)) {
        delete settings['bat' + building.id];
      }

      if (settings.hasOwnProperty('bld_p_' + building.id)) {
        delete settings['bld_p_' + building.id];
      }

      if (settings.hasOwnProperty('bld_s_' + building.id)) {
        delete settings['bld_s_' + building.id];
      }

      if (settings.hasOwnProperty('bld_m_' + building.id)) {
  ...
  function updateStandAloneSettings() {
    ...
    addSetting("buildingStateAll", false); // Collapse or expand settings sections
  ...
  function autoEvolution() {
    ...
    autoUniverseSelection(); // If we have performed a soft reset with a bioseeded ship then we get to choose our planet
      ...
      if (settings.autoAchievements) {
        ...
        for (let i = 0; i < state.raceGroupAchievementList.length; i++) {
          ...
          for (let j = 0; j < raceGroup.length; j++) {
            ...
            if (!race.isMadAchievementUnlocked(achievementLevel) && !race.isEvolutionConditional) {
              // Just ignore conditional races for now
              remainingRace = race;
          ...
          let remainingPercent = remainingAchievements / raceGroup.length; // If this group has the most races left with remaining achievements then target an uncompleted race in this group
        ...
        if (targetedGroup.group != null) {
          state.evolutionTarget = targetedGroup.race;
        }

        if (fallbackGroup.group != null) {
          state.evolutionFallback = fallbackGroup.race;
        }
      }
    ...
    state.evolutions.Rna.click(Math.min(maxRNA, resources.RNA.maxQuantity - resources.RNA.currentQuantity)); // Lets go for our targeted evolution
  ...
  function autoUniverseSelection() {
    ...
    var action = document.getElementById(`uni-${settings.userUniverseTargetName}`); //standard
    //heavy
    //antimatter
    //evil
    //micro
  ...
  function autoPlanetSelection() {
    ...
    if (selectedPlanet === "") {
      selectedPlanet = evolutionPlanetSelection(potentialPlanets, "Oceanic");
    }

    if (selectedPlanet === "") {
      selectedPlanet = evolutionPlanetSelection(potentialPlanets, "Desert");
    }

    if (selectedPlanet === "") {
      selectedPlanet = evolutionPlanetSelection(potentialPlanets, "Volcanic");
    }

    if (selectedPlanet === "") {
      selectedPlanet = evolutionPlanetSelection(potentialPlanets, "Tundra");
    }

    if (selectedPlanet === "") {
      selectedPlanet = evolutionPlanetSelection(potentialPlanets, "Hellscape");
    }

    if (selectedPlanet === "") {
      selectedPlanet = evolutionPlanetSelection(potentialPlanets, "Eden");
  ...
  function autoCraft() {
    ...
    for (let i = 0; i < state.craftableResourceList.length; i++) {
      ...
      if (craftable.autoCraftEnabled) {
        ...
        let tryCraft = true; //console.log("resource: " + craftable.id + ", length: " + craftable.requiredResources.length);
  ...
  function manageSpies() {
    if (!settings.foreignSpyManage) {
      return;
    }

    let foreignVue = getVueById("foreign");

    if (foreignVue === undefined) {
      return;
    }

    if (!game.global.tech['spy'] || !foreignVue.vis()) {
      return;
    }
    ...
    if (!state.spyManager.isUnlocked()) {
      return;
    }
  ...
  function buySpyWithMyLittleEye(foreignVue, govIndex) {
    ...
    if (!settings[`foreignSpy${govIndex}`]) {
      return;
    } // Setting not enabled


    if (game.global.civic.foreign[govProp].occ) {
      return;
    } // Government is occupied


    if (foreignVue.spy_disabled(govIndex)) {
      return;
  ...
  function autoBattle() {
    ...
    while (state.warManager.currentSoldiers < state.warManager.maxSoldiers && resources.Money.storageRatio > settings.foreignHireMercMoneyStoragePercent / 100 && mercenaryCost < settings.foreignHireMercCostLowerThan && resources.Money.currentQuantity > mercenaryCost) {
      state.warManager.hireMercenary();
      mercenaryCost = state.warManager.getMercenaryCost();
      mercenariesHired++; // Just a bit of saftey to ensure that we did actually hire a mercenary
    ...
    let govUnoccupyIndex = -1; // Check if there is an unoccupied foreign power that we can occupy
    ...
    if (govOccupyIndex === -1 && govAttackIndex === -1 && govUnoccupyIndex === -1) {
      return;
    }

    if (state.warManager.maxCityGarrison <= 0 || state.warManager.woundedSoldiers > (1 - settings.foreignAttackHealthySoldiersPercent / 100) * state.warManager.maxCityGarrison || state.warManager.currentCityGarrison < settings.foreignAttackLivingSoldiersPercent / 100 * state.warManager.maxCityGarrison) {
      return;
    ...
    if (!state.warManager.switchToBestAttackType(govOccupyIndex, govAttackIndex, govUnoccupyIndex)) {
      return;
    }

    if (state.warManager.selectedGovAttackIndex === -1) {
      return;
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (Math.min(maxSoldiers, state.warManager.maxCityGarrison) > state.warManager.currentCityGarrison - state.warManager.woundedSoldiers) {
        return;
      } // Log the interaction
      ...
      if (state.warManager.woundedSoldiers > (1 - settings.foreignAttackHealthySoldiersPercent / 100) * state.warManager.maxCityGarrison || state.warManager.currentCityGarrison < settings.foreignAttackLivingSoldiersPercent / 100 * state.warManager.maxCityGarrison) {
        return;
      }
    }
  ...
  function autoHell() {
    if (!state.warManager.isHellUnlocked()) {
      return;
    }
  ...
  function autoJobs() {
    state.jobManager.calculateCraftingMaxs();
    let jobList = state.jobManager.managedPriorityList(); // No jobs unlocked yet
    ...
    let breakpoint1Max = 0; // Cath / Balorg / Imp race doesn't have farmers, unemployed are their farmers
    ...
    let requiredJobs = [];
    let jobAdjustments = [];
    log("autoJobs", "Total employees: " + availableEmployees); // First figure out how many farmers are required

    if (state.jobs.Farmer.isManaged()) {
      if (!state.jobs.Lumberjack.isUnlocked() && !state.jobs.QuarryWorker.isUnlocked() && !state.jobs.CrystalMiner.isUnlocked() && !state.jobs.Scavenger.isUnlocked() && !state.jobs.Miner.isUnlocked() && !state.jobs.CoalMiner.isUnlocked() && !state.jobs.CementWorker.isUnlocked() && !state.jobs.Entertainer.isUnlocked() && !state.jobs.Priest.isUnlocked() && !state.jobs.Professor.isUnlocked() && !state.jobs.Scientist.isUnlocked() && !state.jobs.Banker.isUnlocked() && !state.jobs.Colonist.isUnlocked() && !state.jobs.SpaceMiner.isUnlocked() && !state.jobs.HellSurveyor.isUnlocked() && !state.jobs.Archaeologist.isUnlocked()) {
    ...
    for (let i = 0; i < state.jobManager.maxJobBreakpoints; i++) {
      for (let j = 0; j < jobList.length; j++) {
        const job = jobList[j]; // We've already done the farmer above
        ...
        if (i !== 0) {
          ...
          availableEmployees += requiredJobs[j]; // We have to keep track of craftsmen separately as they have a special max number of total craftsmen
        ...
        if (job === state.jobs.CementWorker) {
          ...
          if (jobsToAssign < currentCementWorkers) {// great, remove workers as we want less than we have
        ...
        availableEmployees -= jobsToAssign; // We have to keep track of craftsmen separately as they have a special max number of total craftsmen
    ...
    let splitJobs = [];
    if (lumberjackIndex !== -1) splitJobs.push({
      jobIndex: lumberjackIndex,
      job: state.jobs.Lumberjack,
      weighting: settings.jobLumberWeighting,
      completed: false
    });
    if (quarryWorkerIndex !== -1) splitJobs.push({
      jobIndex: quarryWorkerIndex,
      job: state.jobs.QuarryWorker,
      weighting: settings.jobQuarryWeighting,
      completed: false
    });
    if (crystalMinerIndex !== -1) splitJobs.push({
      jobIndex: crystalMinerIndex,
      job: state.jobs.CrystalMiner,
      weighting: settings.jobCrystalWeighting,
      completed: false
    });
    if (scavengerIndex !== -1) splitJobs.push({
      jobIndex: scavengerIndex,
      job: state.jobs.Scavenger,
  ...
  function autoTax() {
    ...
    let currentMorale = moraleInstance.current; // main.js -> let mBaseCap = xxxx

    let maxMorale = 100 + state.cityBuildings.Amphitheatre.count + state.cityBuildings.Casino.stateOnCount + state.spaceBuildings.HellSpaceCasino.stateOnCount + state.spaceBuildings.RedVrCenter.stateOnCount * 2 + state.spaceBuildings.AlphaExoticZoo.stateOnCount * 2 + state.spaceBuildings.Alien1Resort.stateOnCount * 2 + state.projects.Monument.level * 2;
    ...
    maxMorale = Math.min(maxMorale, settings.generalMaximumMorale); // Max tax rate calculation
    ...
    if (currentTaxRate < maxTaxRate && (currentTaxRate < settings.generalMinimumTaxRate && resources.Money.storageRatio < 0.98 || currentMorale > settings.generalMinimumMorale && currentMorale >= maxMorale || currentMorale <= settings.generalMinimumMorale && currentTaxRate < 26)) {
      taxVue.add();
    }

    if (currentTaxRate > minTaxRate && (currentTaxRate > settings.generalMinimumTaxRate || resources.Money.storageRatio >= 0.98) && (currentMorale < maxMorale - 1 || currentMorale < settings.generalMinimumMorale && currentTaxRate > 26)) {
      taxVue.sub();
    }
  } //#endregion Auto Tax
  //#region Auto Smelter
  ...
  function autoSmelter() {
    let smelter = state.cityBuildings.Smelter; // No smelter; no auto smelter. No soup for you.
    ...
    if (state.lastSmelterCheckLoop === 0 || state.lastSmelterCheckLoop + 30 <= state.loopCounter) {
      ...
      for (let i = 0; i < steelSmeltingConsumption.length; i++) {
        ...
        if (productionCost.resource.rateOfChange < productionCost.minRateOfChange && productionCost.resource.storageRatio < 0.5 && smelter.smeltingCount(SmelterSmeltingTypes.Steel) > 0) {
          steelAdjustment = -2;
          break;
        }
      }
  ...
  function determineSteelAdjustment() {
    ...
    let smelterSteelCount = state.cityBuildings.Smelter.smeltingCount(SmelterSmeltingTypes.Steel); // The number of buildings hasn't changed so check if we need to adjust. Otherwise continue to updating our numbers

    let maxAllowedSteel = state.cityBuildings.Smelter.maxOperating;
    let currentAvaiableRateOfChange = [];
    let steelSmeltingConsumption = state.cityBuildings.Smelter.smeltingConsumption[SmelterSmeltingTypes.Steel]; // We only care about steel. It isn't worth doing a full generic calculation here
    ...
    if (desiredSteelCount > maxAllowedSteel) {
      desiredSteelCount = maxAllowedSteel;
    }
  ...
  function autoFactory() {
    let factory = state.cityBuildings.Factory; // No factory; no auto factory
    ...
    for (let i = 0; i < allProduction.length; i++) {
      ...
      if (production.factoryAdjustment < 0) {
        state.cityBuildings.Factory.decreaseProduction(production.goods, production.factoryAdjustment * -1);
      }
    ...
    for (let i = 0; i < allProduction.length; i++) {
      ...
      if (production.factoryAdjustment > 0) {
        state.cityBuildings.Factory.increaseProduction(production.goods, production.factoryAdjustment);
      }
    }
  ...
  function autoMiningDroid() {
    let droid = state.spaceBuildings.AlphaMiningDroid; // If not unlocked then nothing to do
  ...
  function autoGraphenePlant() {
    let plant = state.spaceBuildings.AlphaFactory; // If not unlocked then nothing to do
  ...
  function autoMassEjector() {
    ...
    if (state.spaceBuildings.BlackholeMassEjector.stateOnCount === 0) {
      return;
    ...
    if (resourcesByAtomicMass === null) {
      ...
      Object.keys(resources).forEach(resourceKey => {
        ...
        if (resource === resources.Elerium || resource === resource.Infernite) {
          return;
        ...
        if (resource.isEjectable()) {
          resourcesByAtomicMass.push({
            resource: resource,
            requirement: 0
          });
        }
      });
      resourcesByAtomicMass.sort((a, b) => b.resource.atomicMass - a.resource.atomicMass); // Elerium and infernite are always first as they are the exotic resources which are worth the most DE

      resourcesByAtomicMass.unshift({
        resource: resources.Infernite,
        requirement: 0
      });
      resourcesByAtomicMass.unshift({
        resource: resources.Elerium,
        requirement: 0
      });
    }

    let adjustMassEjector = false; // Eject everything!
    ...
    if (state.spaceBuildings.BlackholeMassEjector.stateOnCount >= settings.prestigeWhiteholeEjectAllCount) {
      ...
      resourcesByAtomicMass.forEach(resourceRequirement => {
        ...
        if (resource === resources.Uranium) {
          allowedRatio = 0.2;
        } // Uranium powers buildings which add to storage cap (proxima transfer station) so this flickers if it gets too low


        if (resource === resources.Neutronium) {
          Math.max(allowedRatio, techIds["tech-stabilize_blackhole"].resourceCost(resource.id) / resource.maxQuantity + 0.01);
        }

        if (resource.storageRatio > allowedRatio) {
          let allowedQuantity = allowedRatio * resource.maxQuantity; // If we've got greater than X% left then eject away!
          ...
          if ((resource === resources.Food || resource === resources.Uranium || resource === resources.Neutronium) && resource.currentQuantity / resource.maxQuantity < allowedRatio - 0.01) {
            resourceRequirement.requirement = 0;
  ...
  function isWhiteholePrestigeAvailable() {
    if (getBlackholeMass() < settings.prestigeWhiteholeMinMass) {
      return false;
    }

    if (!techIds["tech-exotic_infusion"].isUnlocked() && !techIds["tech-infusion_check"].isUnlocked() && !techIds["tech-infusion_confirm"].isUnlocked()) {
      return false;
    }
  ...
  function getBlackholeMass() {
    if (!game.global['interstellar'] || !game.global.interstellar['stellar_engine'] || !game.global.interstellar.stellar_engine['mass'] || !game.global.interstellar.stellar_engine['exotic']) {
      return 0;
    }
  ...
  function autoSeederPrestige() {
    ...
    if (!spaceDock.isUnlocked()) {
      return;
    }

    if (spaceDock.count < 1) {
      return;
    }
  ...
  function isBioseederPrestigeAvailable() {
    ...
    if (state.spaceBuildings.GasSpaceDockShipSegment.count < 100) {
      return false;
    }

    if (state.spaceBuildings.GasSpaceDockProbe.count < settings.prestigeBioseedProbes) {
      return false;
    }
  ...
  function autoMarket(bulkSell, ignoreSellRatio) {
    adjustTradeRoutes();
    let m = state.marketManager; // Market has not been unlocked in game yet (tech not researched)
  ...
  function autoBuild() {
    autoGatherResources(); // Space dock is special and has a modal window with more buildings!
    ...
    let buildingList = state.buildingManager.managedPriorityList(); // No buildings unlocked yet
    ...
    for (let i = 0; i < buildingList.length; i++) {
      ...
      if (building === state.cityBuildings.CoalPower) {
        ...
        if (!isLowPlasmidCount()) {
          ...
          if (buildIfEnoughProduction(building, resources.Coal, 0.5)) {
            // If we don't have plasmids then have to go much lower
            return;
          }
        }
      }
  ...
  function autoResearch() {
    let items = document.querySelectorAll('#tech .action'); // Check for active research triggers
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      let click = false; // Block research that conflics with active triggers, but never block research that is wanted by an active trigger
      // @ts-ignore
      ...
      if (itemId === "tech-stabilize_blackhole" && settings.prestigeWhiteholeStabiliseMass && getBlackholeMass() < settings.prestigeWhiteholeMinMass) {
        ...
        click = true;
      } else if (itemId === "tech-exotic_infusion" || itemId === "tech-infusion_check" || itemId === "tech-infusion_confirm" || itemId === "tech-stabilize_blackhole" || itemId === "tech-dial_it_to_11" || itemId === "tech-limit_collider" || itemId === "tech-demonic_infusion" || itemId == "tech-dark_bomb") {
        // Don't click any of the whitehole / cataclysm reset options without user consent... that would be a dick move, man.
        continue;
      }

      if (itemId !== "tech-anthropology" && itemId !== "tech-fanaticism" && itemId !== "tech-wc_reject" && itemId !== "tech-wc_money" && itemId !== "tech-wc_morale" && itemId !== "tech-wc_conquest" && itemId !== "tech-study" && itemId !== "tech-deify") {
        click = true;
      } else {
  ...
  function autoArpa() {
    let projectList = state.projectManager.managedPriorityList(); // Special autoSpace logic. If autoSpace is on then ignore other ARPA settings and build once MAD has been researched
    ...
    for (let i = 0; i < projectList.length; i++) {
      const project = projectList[i]; // Only level up to user defined max
    ...
    for (let i = 0; i < projectList.length; i++) {
      ...
      for (let j = 0; j < project.resourceRequirements.length; j++) {
        ...
        if (onePercentOfRequirementQuantity === 0) {
          log("autoARPA", "continue: cost is zero");
          continue;
        } // Monument can be made of different things. Sometimes these requirements will be zero.
  ...
  function autoBuildingPriority() {
    let availablePowerNode = document.querySelector('#powerMeter'); // Only start doing this once power becomes available. Isn't useful before then

    if (availablePowerNode === null) {
      return;
    }

    let buildingList = state.buildingManager.managedStatePriorityList(); // No buildings unlocked yet
    ...
    for (let i = 0; i < buildingList.length; i++) {
      ...
      for (let j = 0; j < building.consumption.resourceTypes.length; j++) {
        let resourceType = building.consumption.resourceTypes[j]; // Mass driver effect
    ...
    for (let i = 0; i < buildingList.length; i++) {
      ...
      for (let j = 0; j < building.count; j++) {
        ...
        for (let k = 0; k < building.consumption.resourceTypes.length; k++) {
          let resourceType = building.consumption.resourceTypes[k]; // TODO: Implement minimum rates of change for each resource
          ...
          if (resourceType.rate > 0) {
            let isStorageAvailable = false; // If we have more than xx% of our storage then its ok to lose some resources.
            ...
            } else if (resourceType.resource === resources.Coal || resourceType.resource === resources.Oil || resourceType.resource === resources.Uranium || resourceType.resource === resources.Helium_3 || resourceType.resource === resources.Elerium || resourceType.resource === resources.Deuterium) {
  ...
  function addToStorageAdjustments(storageChanges, resource, requiredCrates, requiredContainers) {
    if (resource.currentCrates !== requiredCrates || resource.currentContainers !== requiredContainers) {
      ...
      if (crates !== 0 || containers !== 0) {
        storageChanges.adjustments.push({
          resource: resource,
          cratesAdjustment: crates,
          containersAdjustment: containers
        });
  ...
  function autoStorage() {
    ...
    resources.Crates.resourceRequirements.forEach(requirement => numberOfCratesWeCanBuild = Math.min(numberOfCratesWeCanBuild, requirement.resource.currentQuantity / requirement.quantity));
    resources.Containers.resourceRequirements.forEach(requirement => numberOfContainersWeCanBuild = Math.min(numberOfContainersWeCanBuild, requirement.resource.currentQuantity / requirement.quantity));
    let storageChanges = {
      ...
      storageList.forEach(resource => {
        ...
        if (settings.storageLimitPreMad) {
          if (resource === resources.Steel) {
            requiredCrates = Math.max(50, requiredCrates);
          }

          if (resource === resources.Aluminium) {
            requiredCrates = Math.max(20, requiredCrates);
          }
          ...
          if (resource === resources.Alloy) {
            requiredCrates = Math.max(20, requiredCrates);
          }

          if (resource === resources.Polymer) {
            requiredCrates = Math.max(5, requiredCrates);
          }
        }
  ...
  function adjustTradeRoutes() {
    ...
    let resourcesToTrade = []; // Calculate the resources and money that we would have if we weren't trading anything on the market
    ...
    let minimumAllowedMoneyPerSecond = Math.max(settings.tradeRouteMinimumMoneyPerSecond, settings.tradeRouteMinimumMoneyPercentage / 100 * currentMoneyPerSecond); //console.log("minimum money per second: " + minimumAllowedMoneyPerSecond + " based on current money per second of " + currentMoneyPerSecond)

    while (findArrayIndex(resourcesToTrade, "completed", false) != -1) {
      for (let i = 0; i < resourcesToTrade.length; i++) {
        const resourceToTrade = resourcesToTrade[i]; //console.log(state.loopCounter + " " + resourceToTrade.resource.id + " testing...")
        ...
        if (!resourceToTrade.completed && tradeRoutesUsed < maxTradeRoutes && resourceToTrade.requiredTradeRoutes > requiredTradeRoutes[resourceToTrade.index] && currentMoneyPerSecond - resourceToTrade.resource.currentTradeRouteBuyPrice > minimumAllowedMoneyPerSecond) {
          currentMoneyPerSecond -= resourceToTrade.resource.currentTradeRouteBuyPrice;
          tradeRoutesUsed++;
          requiredTradeRoutes[resourceToTrade.index]++; //console.log(state.loopCounter + " " + resourceToTrade.resource.id + " adding trade route - " + resourceToTrade.index)

          continue;
        } // We're buying enough resources now or we don't have enough money to buy more anyway


        if (resourceToTrade.requiredTradeRoutes === requiredTradeRoutes[resourceToTrade.index] || currentMoneyPerSecond - resourceToTrade.resource.currentTradeRouteBuyPrice < minimumAllowedMoneyPerSecond) {
        ...
        if (resourceToTrade.requiredTradeRoutes > requiredTradeRoutes[resourceToTrade.index]) {
          ...
          for (let i = tradableResources.length - 1; i >= 0; i--) {
            ...
            let reducedMoneyPerSecond = 0; // We can't remove it if we're not selling it or if we are looking at the same resource
            ...
            while (currentRequired < 0 && resourceToTrade.requiredTradeRoutes > requiredTradeRoutes[resourceToTrade.index]) {
              ...
              if (currentMoneyPerSecond - reducedMoneyPerSecond - resourceToTrade.resource.currentTradeRouteBuyPrice > minimumAllowedMoneyPerSecond) {
                ...
                currentMoneyPerSecond -= resourceToTrade.resource.currentTradeRouteBuyPrice; //console.log(state.loopCounter + " " + resourceToTrade.resource.id + " current money per second: " + currentMoneyPerSecond);
  ...
  function verifyGameActions() {
    ...
    let p_structs = ['city:apartment', 'int_alpha:habitat', 'int_alpha:luxury_condo', 'spc_red:spaceport', 'int_alpha:starport', 'int_blackhole:s_gate', 'gxy_gateway:starbase', 'gxy_gateway:ship_dock', 'int_neutron:stellar_forge', 'int_neutron:citadel', 'city:coal_mine', 'spc_moon:moon_base', 'spc_red:red_tower', 'spc_home:nav_beacon', 'int_proxima:xfer_station', 'gxy_stargate:telemetry_beacon', 'int_nebula:nexus', 'gxy_stargate:gateway_depot', 'spc_dwarf:elerium_contain', 'spc_gas:gas_mining', 'spc_belt:space_station', 'spc_gas_moon:outpost', 'gxy_gorddon:embassy', 'gxy_gorddon:dormitory', 'gxy_alien1:resort', 'spc_gas_moon:oil_extractor', 'int_alpha:int_factory', 'city:factory', 'spc_red:red_factory', 'spc_dwarf:world_controller', 'prtl_fortress:turret', 'prtl_badlands:war_drone', 'city:wardenclyffe', 'city:biolab', 'city:mine', 'city:rock_quarry', 'city:cement_plant', 'city:sawmill', 'city:mass_driver', 'int_neutron:neutron_miner', 'prtl_fortress:war_droid', 'prtl_pit:soul_forge', 'gxy_chthonian:excavator', 'int_blackhole:far_reach', 'prtl_badlands:sensor_drone', 'prtl_badlands:attractor', 'city:metal_refinery', 'gxy_stargate:gateway_station', 'gxy_alien1:vitreloy_plant', 'gxy_alien2:foothold', 'gxy_gorddon:symposium', 'int_blackhole:mass_ejector', 'city:casino', 'spc_hell:spc_casino', 'prtl_fortress:repair_droid', 'gxy_stargate:defense_platform', 'prtl_pit:gun_emplacement', 'prtl_pit:soul_attractor', 'int_sirius:ascension_trigger']; // Perform the check
  ...
  function automate() {
    ...
    updateState();
    updateUI(); // The user has turned off the master toggle. Stop taking any actions on behalf of the player.
  ...
  setInterval(shortLoop, 50); //#endregion Main Loop
  //#region UI
  ...
  function addScriptStyle() {
    ...
    css.type = 'text/css';
    css.appendChild(document.createTextNode(styles)); // Append style to html head
  ...
  function buildSettingsSection(sectionId, sectionName, resetFunction, updateSettingsContentFunction) {
    ...
    scriptContentNode.append('<div id="script_' + sectionId + 'Settings" style="margin-top: 10px;">' + '<h3 id="' + sectionId + 'SettingsCollapsed" class="script-collapsible text-center has-text-success">' + sectionName + ' Settings</h3>' + '<div class="script-content">' + '<div style="margin-top: 10px;"><button id="script_reset' + sectionId + '" class="button">Reset ' + sectionName + ' Settings</button></div>' + '<div style="margin-top: 10px; margin-bottom: 10px;" id="script_' + sectionId + 'Content"></div>' + '</div>' + '</div>');
    ...
    if (!settings[sectionId + "SettingsCollapsed"]) {
      ...
      let content = element.nextElementSibling; //@ts-ignore

      content.style.display = "block";
    }

    $("#script_reset" + sectionId).on("click", function () {
      genericResetFunction(resetFunction, sectionName);
    });
  }
  ...
  function buildSettingsSection2(parentNode, isMainSettings, sectionId, sectionName, resetFunction, updateSettingsContentFunction) {
    ...
    if (isMainSettings) {
      let headerNode = $('<div id="script_' + mainSectionId + 'Settings" style="margin-top: 2px;">' + '<h3 id="' + mainSectionId + 'SettingsCollapsed" class="script-collapsible text-center has-text-success">' + sectionName + ' Settings</h3>' + '</div>');
      contentContainerNode = $('<div class="script-content">' + '<div style="margin-top: 2px;"><button id="script_reset' + mainSectionId + '" class="button">Reset ' + sectionName + ' Settings</button></div>' + '</div>');
      headerNode.append(contentContainerNode);
      parentNode.append(headerNode);
      $("#script_reset" + mainSectionId).on("click", function () {
        genericResetFunction(resetFunction, sectionName);
      });
    }
  ...
  function updateGeneralPreTable() {
    let currentNode = $('#script_generalContent'); // Add the pre table section

    currentNode.append('<div style="margin-top: 10px; margin-bottom: 10px;" id="script_generalPreTable"></div>'); // Add any pre table settings
  ...
  function updatePrestigeSettingsContent(isMainSettings) {
    ...
    currentNode.empty().off("*"); // Foreign powers panel

    let prestigeHeaderNode = $(`<div id="script_${secondaryPrefix}prestige"></div>`);
    currentNode.append(prestigeHeaderNode); // MAD

    addStandardSectionHeader1(prestigeHeaderNode, "Mutual Assured Destruction");
    addStandardSectionSettingsToggle2(secondaryPrefix, prestigeHeaderNode, 0, "autoMAD", "Perform MAD prestige", "MAD prestige once MAD has been researched and all soldiers are home"); // Bioseed
    ...
    addStandardSectionSettingsNumber2(secondaryPrefix, prestigeHeaderNode, 1, "prestigeBioseedProbes", "Required probes", "Required number of probes before launching bioseeder ship"); // Whitehole
  ...
  function updateGovernmentSettingsContent(isMainSettings) {
    ...
    currentNode.empty().off("*"); // Add the pre table section

    currentNode.append(`<div id="script_${secondaryPrefix}governmentPreTable"></div>`); // Add any pre table settings
    ...
    addStandardSectionSettingsToggle2(secondaryPrefix, preTableNode, 0, "govManage", "Manage changes of government", "Manage changes of government when they become available"); // Government selector
  ...
  function updateEvolutionSettingsContent() {
    ...
    currentNode.empty().off("*"); // Target universe
    ...
    selectNode.append(node); // TODO: Add in auto when script manages this
    ...
    selectNode.on('change', function () {
      ...
      updateSettingsFromState(); //console.log("Chosen universe target of " + value);

      let content = document.querySelector('#script_evolutionSettings .script-content'); // @ts-ignore

      content.style.height = null; // @ts-ignore
    ...
    selectNode.on('change', function () {
      ...
      updateSettingsFromState(); //console.log("Chosen evolution target of " + value);
  ...
  function updateTriggerPreTable() {
    let currentNode = $('#script_triggerContent'); // Add the pre table section

    currentNode.append('<div style="margin-top: 10px; margin-bottom: 10px;" id="script_triggerPreTable"></div>'); // Add any pre table settings
    ...
    $("#script_trigger_add").on("click", addTriggerSetting); //addStandardSectionSettingsNumber(preTableNode, "jobLumberWeighting", "Final Lumberjack Weighting", "AFTER allocating breakpoints this weighting will be used to split lumberjacks, quarry workers and scavengers");
  }
  ...
  function addTriggerSetting() {
    ...
    let content = document.querySelector('#script_triggerSettings .script-content'); // @ts-ignore
  ...
  function updateTriggerTable() {
    let currentScrollPosition = document.documentElement.scrollTop || document.body.scrollTop;
    let currentNode = $('#script_triggerContent');
    currentNode.append(`<table style="width:100%">
    ...
    for (let i = 0; i < state.triggerManager.priorityList.length; i++) {
      const trigger = state.triggerManager.priorityList[i]; //let triggerElement = $('#script_trigger_' + trigger.seq);
    ...
    $('#script_triggerTableBody').sortable({
      ...
      update: function () {
        let triggerIds = $('#script_triggerTableBody').sortable('toArray', {
          attribute: 'value'
        });

        for (let i = 0; i < triggerIds.length; i++) {
          const seq = parseInt(triggerIds[i]); // Trigger has been dragged... Update all trigger priorities
  ...
  function buildTriggerType(trigger) {
    let triggerElement = $('#script_trigger_' + trigger.seq); // Trigger Type
    ...
    typeSelectNode.append(typeOptionNode); // selected = trigger.type === "bld" ? ' selected="selected"' : "";
  ...
  function buildTriggerRequirementType(trigger) {
    ...
    if (trigger.type === "tech") {
      ...
      typeSelectNode.append(typeOptionNode); // selected = trigger.type === "researched" ? ' selected="selected"' : "";
    ...
    if (trigger.type === "bld") {// TODO: Building triggers
    }
  }
  /**
   * @param {Trigger} trigger
  ...
  function buildTriggerRequirementCount(trigger) {// let triggerElement = $('#script_trigger_' + trigger.seq);
  ...
  function buildTriggerActionType(trigger) {
    ...
    if (trigger.type === "tech") {
      ...
      typeSelectNode.append(typeOptionNode); // selected = trigger.type === "build" ? ' selected="selected"' : "";
  ...
  function buildTriggerActionCount(trigger) {//let triggerElement = $('#script_trigger_' + trigger.seq);
  ...
  function updateResearchSettingsContent() {
    ...
    currentNode.empty().off("*"); // Theology 1
    ...
    selectNode.on('change', function () {
      ...
      updateSettingsFromState(); //console.log("Chosen theology 1 target of " + value);
    }); // Theology 2
    ...
    selectNode.on('change', function () {
      ...
      updateSettingsFromState(); //console.log("Chosen theology 2 target of " + value);
    }); // Unification
    ...
    selectNode.on('change', function () {
      ...
      updateSettingsFromState(); //console.log("Chosen unification target of " + value);
    });
    document.documentElement.scrollTop = document.body.scrollTop = currentScrollPosition;
  }
  ...
  function updateWarSettingsContent(isMainSettings) {
    ...
    updateForeignPowerPanel(secondaryPrefix, foreignPowerNode, 2); // Campaign panel
    ...
    currentNode.append(`<table style="width:100%"><tr><th class="has-text-warning" style="width:25%">Campaign</th><th class="has-text-warning" style="width:25%">Minimum Attack Rating</th><th class="has-text-warning" style="width:25%">Maximum Rating to Send</th><th class="has-text-warning" style="width:25%"></th></tr>
    ...
    warTableBody.append($(newTableBodyText)); // Build campaign settings rows
  ...
  function updateHellSettingsContent(isMainSettings) {
    ...
    currentNode.empty().off("*"); // Hell panel

    let hellHeaderNode = $(`<div id="script_${secondaryPrefix}hell"></div>`);
    currentNode.append(hellHeaderNode); // Entering Hell
    ...
    addStandardSectionSettingsNumber2(secondaryPrefix, hellHeaderNode, 0, "hellMinSoldiersPercent", "Alive soldier percentage for entering hell (only prevents entering, walls also have to be 100%)", "Don't enter hell if too many soldiers are dead, but don't get out"); // Hell Garrison
    ...
    addStandardSectionSettingsNumber2(secondaryPrefix, hellHeaderNode, 0, "hellLowWallsMulti", "Garrison bolster factor for damaged walls", "Multiplies target defense rating by this when close to 0 wall integrity, half as much increase at half integrity"); // Patrol size
    ...
    addStandardSectionSettingsNumber2(secondaryPrefix, hellHeaderNode, 1, "hellBolsterPatrolPercentBottom", "Full patrol rating increase below this home garrison fill percent", "This is the lower number"); // Attractors
  ...
  function updateMarketPreTable() {
    let currentNode = $('#script_marketContent'); // Add the pre table section

    currentNode.append('<div style="margin-top: 10px; margin-bottom: 10px;" id="script_marketPreTable"></div>'); // Add any pre table settings
  ...
  function updateMarketTable() {
    ...
    tableBodyNode.append($(newTableBodyText)); // Build all other markets settings rows
    ...
    $('#script_marketTableBody').sortable({
      ...
      update: function () {
        let marketIds = $('#script_marketTableBody').sortable('toArray', {
          attribute: 'value'
        });
  ...
  function buildMarketSettingsToggle(resource, property, toggleId, syncToggleId, oppositeProperty, oppositeToggleId, oppositeSyncToggleId) {
    ...
    toggle.on('change', function (e) {
      ...
      updateSettingsFromState(); //console.log(resource.name + " changed enabled to " + state);
    });
    return toggle;
  }
  /**
  ...
  function updateStoragePreTable() {
    let currentNode = $('#script_storageContent'); // Add the pre table section
    ...
    currentNode.append('<div style="margin-top: 10px; margin-bottom: 10px;" id="script_storagePreTable"></div>'); // Add any pre table settings
  ...
  function updateStorageTable() {
    let currentNode = $('#script_storageContent');
    currentNode.append(`<table style="width:100%"><tr><th class="has-text-warning" style="width:20%">Resource</th><th class="has-text-warning" style="width:20%">Enabled</th><th class="has-text-warning" style="width:20%">Weighting</th><th class="has-text-warning" style="width:20%">Max Crates</th><th class="has-text-warning" style="width:20%">Max Containers</th></tr>
    ...
    tableBodyNode.append($(newTableBodyText)); // Build all other storages settings rows
    ...
    $('#script_storageTableBody').sortable({
      ...
      update: function () {
        let storageIds = $('#script_storageTableBody').sortable('toArray', {
          attribute: 'value'
        });
  ...
  function updateMinorTraitTable() {
    let currentNode = $('#script_minorTraitContent');
    currentNode.append(`<table style="width:100%"><tr><th class="has-text-warning" style="width:20%">Minor Trait</th><th class="has-text-warning" style="width:20%">Enabled</th><th class="has-text-warning" style="width:20%">Weighting</th><th class="has-text-warning" style="width:40%"></th></tr>
    ...
    tableBodyNode.append($(newTableBodyText)); // Build all other minorTraits settings rows
    ...
    $('#script_minorTraitTableBody').sortable({
      ...
      update: function () {
        let minorTraitNames = $('#script_minorTraitTableBody').sortable('toArray', {
          attribute: 'value'
        });
  ...
  function updateProductionPreTableSmelter() {
    let currentNode = $('#script_productionContent'); // Add the pre table section

    currentNode.append('<div style="margin-top: 10px; margin-bottom: 10px;" id="script_productionPreTableSmelter"></div>'); // Add any pre table settings

    let preTableNode = $('#script_productionPreTableSmelter');
    addStandardHeading(preTableNode, "Smelter"); //addStandardSectionSettingsToggle(preTableNode, "productionMoneyIfOnly", "Override and produce money if we can't fill factories with other production", "If all other production has been allocated and there are leftover factories then use them to produce money");
  }

  function updateProductionTableSmelter() {
    let currentNode = $('#script_productionContent');
    currentNode.append(`<table style="width:100%"><tr><th class="has-text-warning" style="width:25%">Fuel</th><th class="has-text-warning" style="width:75%"></th></tr>
    ...
    tableBodyNode.append($(newTableBodyText)); // Build all other productions settings rows
    ...
    $('#script_productionTableBodySmelter').sortable({
      ...
      update: function () {
        let fuelIds = $('#script_productionTableBodySmelter').sortable('toArray', {
          attribute: 'value'
        });
  ...
  function updateProductionPreTableFactory() {
    ...
    currentNode.append('<div style="margin-top: 10px; margin-bottom: 10px;" id="script_productionPreTableFactory"></div>'); // Add any pre table settings
  ...
  function updateProductionTableFactory() {
    let currentNode = $('#script_productionContent');
    currentNode.append(`<table style="width:100%"><tr><th class="has-text-warning" style="width:20%">Resource</th><th class="has-text-warning" style="width:20%">Enabled</th><th class="has-text-warning" style="width:20%">Weighting</th><th class="has-text-warning" style="width:40%"></th></tr>
    ...
    let newTableBodyText = "";
    let productionSettings = state.cityBuildings.Factory.productionOptions;
    productionSettings.sort(function (a, b) {
      return a.seq - b.seq;
    });
  ...
  function updateJobPreTable() {
    let currentNode = $('#script_jobContent'); // Add the pre table section

    currentNode.append('<div style="margin-top: 10px; margin-bottom: 10px;" id="script_jobPreTable"></div>'); // Add any pre table settings
  ...
  function updateJobTable() {
    let currentScrollPosition = document.documentElement.scrollTop || document.body.scrollTop;
    let currentNode = $('#script_jobContent');
    currentNode.append(`<table style="width:100%"><tr><th class="has-text-warning" style="width:25%">Job</th><th class="has-text-warning" style="width:25%">1st Pass Max</th><th class="has-text-warning" style="width:25%">2nd Pass Max</th><th class="has-text-warning" style="width:25%">Final Max</th></tr>
    ...
    $('#script_jobTableBody').sortable({
      ...
      update: function () {
        let jobIds = $('#script_jobTableBody').sortable('toArray', {
          attribute: 'value'
        });
  ...
  function buildJobSettingsToggle(job) {
    ...
    toggle.on('change', function (e) {
      ...
      updateSettingsFromState(); //console.log(job._originalName + " changed state to " + state);
    });
    return toggle;
  }
  /**
  ...
  function updateBuildingPreTable() {// let currentNode = $('#script_marketContent');
    // currentNode.append(
  ...
  function updateBuildingTable() {
    ...
    $('#script_buildingTableBody').sortable({
      ...
      update: function () {
        let buildingIds = $('#script_buildingTableBody').sortable('toArray', {
          attribute: 'value'
        });
  ...
  function filterBuildingSettingsTable() {
    // Declare variables
    let input = document.getElementById("script_buildingSearch"); //@ts-ignore

    let filter = input.value.toUpperCase();
    let table = document.getElementById("script_buildingTableBody");
    let trs = table.getElementsByTagName("tr"); // Loop through all table rows, and hide those who don't match the search query
  ...
  function buildBuildingEnabledSettingsToggle(building) {
    ...
    toggle.on('change', function (e) {
      let input = e.currentTarget.children[0];
      let state = input.checked;
      building.autoBuildEnabled = state; //$('#script_bat1_' + building.settingId + ' input').checked = state; // Update the on-building toggle
      ...
      updateSettingsFromState(); //console.log(building.name + " changed enabled to " + state);
    });
    return toggle;
  }
  /**
  ...
  function buildBuildingStateSettingsToggle(building) {
    ...
    toggle.on('change', function (e) {
      ...
      updateSettingsFromState(); //console.log(building.name + " changed state to " + state);
    });
    return toggle;
  }
  /**
  ...
  function updateProjectPreTable() {
    let currentNode = $('#script_projectContent'); // Add the pre table section

    currentNode.append('<div style="margin-top: 10px; margin-bottom: 10px;" id="script_projectPreTable"></div>'); // Add any pre table settings
  ...
  function updateProjectTable() {
    let currentNode = $('#script_projectContent');
    currentNode.append(`<table style="width:100%"><tr><th class="has-text-warning" style="width:25%">Project</th><th class="has-text-warning" style="width:25%">Max Build</th><th class="has-text-warning" style="width:25%">Ignore Min Money</th><th class="has-text-warning" style="width:25%"></th></tr>
    ...
    tableBodyNode.append($(newTableBodyText)); // Build all other projects settings rows
    ...
    $('#script_projectTableBody').sortable({
      ...
      update: function () {
        let projectIds = $('#script_projectTableBody').sortable('toArray', {
          attribute: 'value'
        });
  ...
  function buildProjectSettingsToggle(project) {
    ...
    toggle.on('change', function (e) {
      let input = e.currentTarget.children[0];
      let state = input.checked;
      project.autoBuildEnabled = state; // @ts-ignore

      document.querySelector('#script_arpa1_' + project.id + ' input').checked = state;
      updateSettingsFromState(); //console.log(project.name + " changed enabled to " + state);
    });
    return toggle;
  }
  /**
  ...
  function updateLoggingSettingsContent(isMainSettings) {
    ...
    currentNode.append(`<div id="script_${secondaryPrefix}loggingPreTable"></div>`); // Add any pre table settings
  ...
  function addOptionUiClickHandler(optionNode, modalTitle, buildOptionsFunction) {
    optionNode.on('click', function () {
      ...
      modalBody.empty().off("*");
      buildOptionsFunction(modalBody); // Show modal
...
`); // Append the script modal to the document
    ...
    $(document.body).append(modal); // Add the script modal close button action
  ...
  function updateUI() {
    ...
    if (document.getElementById("s-quick-prestige-options") === null) {
      createQuickOptions("s-quick-prestige-options", "Prestige", buildPrestigeSettings);
    }
  ...
  function createBuildingToggle(building) {
    ...
    toggle.on('change', function (e) {
      let input = e.currentTarget.children[0];
      let state = input.checked;
      building.autoBuildEnabled = state; //$('#script_bat2_' + building.settingId + ' input').checked = state; // Update the settings-building toggle
  ...
  function createMarketToggle(resource) {
    ...
    toggleBuy.on('change', function (e) {
      ...
      updateSettingsFromState(); //console.log(state);
    });
  ...
  $(window).on('blur', function (e) {
    let keyboardEvent = document.createEvent("KeyboardEvent"); // @ts-ignore
  ...
  $(document).ready(function () {
    let autoEvolveScriptText = `
        ...
        `;
    $('<script>').attr('type', 'module').text(autoEvolveScriptText).appendTo('head');
  }); // @ts-ignore
})($);
