...
const changeList = [
    {
        ...
        date: `4/24/2020`,
        changes: [
            `Farming has been redesigned.`,
            `农业系统重新设计。`,
            `Farms no longer directly generate food, Farmer now generate food in combination with farms.`,
            `农场不再直接生产食物，必须在农场生产。`,
            `Added energizer feat for ascending without building any thermal collectors.`,
            `增加一项能量壮举，不造集热器飞升。`,
            `Bad traids no longer count for ascension sprcies creator complexity.`,
            `负面特质不纳入基因奢侈税计算。`,
            `Dreaded achievement will now unlock if you never researched dreadnoughts.`,
            `畏惧成就的解锁条件是不研究无畏舰。（以前是不建造无畏舰？）`,
            `Fixed issue with some feats spamming the log due to micro achievements.`,
            `修复了读取微型宇宙成就时导致的日志堆积错误。（喵队长醒醒！王师来了！你有救了！）`,
            `Fixed base training rate of soldiers.`,
            `修复了士兵的基准训练速度。`,
            `Fixed bug with diverse trait that made it do the poopsite of what it was suppose to do`,
            `修复了一些特质不作为或乱作为的BUG。`,
